...
const hash_dir = crypto.createHash('md5').update(loadConfig.cwd()).digest("hex")
const formidable = require('koa-formidable')
const osTmp = require('os').tmpdir()
const tmpDir = path.join(osTmp, hash_dir)
mkdir.sync(tmpDir)
const root = (require('os').platform == "win32") ? loadConfig.cwd().split(path.sep)[0] : "/"
...
function loadFile(p, throwErr = true) {
  ...
  return new Promise((resolve, reject) => {
    fs.stat(util.makeAbsolute(p), (err, stats) => {
      if (err) {
...
router.get('/', function *() {
  let [config, rootFn] = yield [loadConfig.appConfig(), util.loadTemplate('index')]
...
router.get('/appservice', function *() {
  let [config, files, serviceFn] = yield [loadConfig.appConfig(), util.globJSfiles(), util.loadTemplate('service')]
...
router.get('/app/(.*)', function* () {
  ...
  if (/\.(wxss|js)$/.test(file)) {
    ...
    let [config, exists] = yield [loadConfig.appConfig(), util.exists(file)]
    if (!exists) {
      this.status = 404
    ...
    if (exists) {
      yield send(this, file, { root: loadConfig.cwd() })
    } else {
      this.status = 404
      throw new Error(`File: ${file} not found`)
    }
